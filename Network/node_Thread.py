...
class Node:
    ...
    def __init__(self, name, network, delay_range=(2, 7)): #is_coordinator=False
        ...
        # self.logger = self.setup_logs(name)
    ...
    def receive_transaction(self, transaction, sender=None, delay=None):
        ...
        with self.transaction_lock:
            ...
            self.logger.info(f"{datetime.now().strftime('%H:%M:%S.%f')} - {self.name} received Transaction ID {transaction.txid} after delay {delay} from {sender.name}")
            ...
            with self.tips_lock:
                if not transaction.children and transaction not in self.tips:
                    ...
                    self.logger.info(f"{datetime.now().strftime('%H:%M:%S.%f')} - Transaction ID : {transaction.txid} do not have childern, thats why added to tip of {self.name} ")
                    self.tips.append(transaction)
                for parent in transaction.parent_transactions:
                    if parent in self.tips:
                        self.tips.remove(parent)
                        print(f" Transaction {parent.txid} is no longer a tip for {self.name}")
                        self.logger.info(f"{datetime.now().strftime('%H:%M:%S.%f')} - Transaction {parent.txid} is no longer a tip for {self.name} ")
